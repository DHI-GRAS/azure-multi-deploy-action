...
exports.default = async (pkg, pullNumber) => {
    try {
        ...
        const { stdout: uploadOut, stderr: uploadErr } = await (0, child_process_promise_1.exec)(`cd ${pkg.path}/dist/ && az storage azcopy copy ./\\* 'https://${stagName}.blob.core.windows.net/$web/${slotName}' --recursive --container \\$web --account-name ${stagName} --source ./\\* --destination ${slotName} --auth-mode key`).catch((err) => {
            throw Error(err);
        });
        if (stdout)
